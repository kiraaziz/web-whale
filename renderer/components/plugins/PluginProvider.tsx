...
export default function PluginProvider() {
    const { templates, isLoadingTemplates, fetchError, handleFileUpload, isUploading, fetchTemplates } = usePlugins();
            ...
            {isLoadingTemplates && templates.length === 0 ? (
                ...
                templates.length === 0 ? (
                    ...
                    <div className='w-full grid grid-cols-3 gap-3'>
                        {templates.map((template, index) => <PluginCart key={template.base} template={template} reavlidate={fetchTemplates} />)}
                    </div>
                )}
        </AlertBox>
    )
